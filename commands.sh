...
# which sends signal msgs to _reminderbot.  The signal msgs contain information for one or more
...
function cronjobSubroutine {
    ...
    if [[ "$(echo ${payload,,} | sed -r 's|^(tomorrow).*|\1|')" == "tomorrow" ]] || [[ "$(echo ${payload,,} | sed -r 's|^(tmrw).*|\1|')" == "tmrw" ]] ; then    # Case: tomorrow to do something ...
        ...
        task=$(echo ${payload} | sed -r 's|^in ([0-9]+.*(d\|h\|m)) (.*)|\3|' | sed 's|^[ ]*||' | sed 's|[ ]*$||')       # Get the task.

        if [ ! ${days} ] && [ ! ${hours} ] && [ ! ${minutes} ] ; then                                                   # If d,h,m are all missing, return immediately.

#!/usr/bin/env bash
# ReminderBot ~ Subroutines/Commands
...
BOT_NICK="$(grep -P "BOT_NICK=.*" ${DIR}/ReminderBot.sh | cut -d '=' -f 2- | tr -d '"')"

if [ "${chan}" = "${BOT_NICK}" ] ; then chan="${nick}" ; fi

###################################################  Settings  ####################################################

DIR_PATH="${DIR}"                                       # Path to ReminderBot.
CURRENT_YEAR='18'
...
DEBUG='#'                                               # Comment this line to display debug msgs.
...
# In ReminderBot.sh, an endless while loop looks for a tmp file in ${DIR_PATH}/tasks.
# As soon as a tmp file is generated by a cronjob execution, ReminderBot.sh runs signalSubroutine
# which sends signal msgs to ReminderBot.  Signal msgs contain information about one or more
...
function cronjobSubroutine {
    ...
    if [[ "$(echo ${payload,,} | sed -r 's|^(tomorrow).*|\1|')" == "tomorrow" ]] || [[ "$(echo ${payload,,} | sed -r 's|^(tmrw).*|\1|')" == "tmrw" ]] ; then    # Case: tomorrow to do something ...
        ...
        uuid="${UUIDGEN}"
        time_sched="$(date)"
        (crontab -l ; echo "${cronjob} echo \"${uuid}: ${time_sched}, ${chan}, ${nick}, ${task}\" >> ${DIR_PATH}/tasks/tmp") | crontab -       # Create a new cronjob entry.  (NOTE: when a cronjob is executed, ../tmp will be created.  Then, _ReminderBot will send contents of tmp to ReminderBot, and remove tmp.)
        if [ "$(echo $?)" == "1" ] ; then return 1 ; fi
...
# This subroutine displays documentation for ReminderBot's functionalities.

function helpSubroutine {
    say ${chan} "${nick}: I will remind you of stuff!  DISCLAIMER: I am not liable for your forgetfulness."
    say ${chan} 'usage: remind me in 3d2h1m ... | remind me in 1m2h ... | remind me in 1d ...'
    ...
    say ${chan} 'To remove reminders, try ==> !reminders'
}
...
if has "${msg}" "^!ReminderBot$" || has "${msg}" "^ReminderBot: help$" ; then
    helpSubroutine
...
# Alive.
    ...
    str1='running! '
    str2=$(ps aux | grep ./ReminderBot | head -n 1 | awk '{ print "[%CPU "$3"]", "[%MEM "$4"]", "[VSZ "$5"]" }')
...
elif has "${msg}" "^ReminderBot: alive(\?)?$" ; then
    str1='running! '
    str2=$(ps aux | grep ./ReminderBot | head -n 1 | awk '{ print "[%CPU "$3"]", "[%MEM "$4"]", "[VSZ "$5"]" }')
    str3=" [#REM $(crontab -l | tail -n +3 | wc -l)]"
    str4=" [SIZE $(ls -lash | head -n 1 | cut -d ' ' -f2-)]"
...
elif has "${msg}" "^ReminderBot: source$" ; then
    say ${chan} "Try -> https://github.com/kimdj/ReminderBot -OR- ${DIR_PATH}"
...
# Handle incoming msg from self (ReminderBot => ReminderBot).

elif has "${msg}" "^!signal " && [[ ${nick} = "ReminderBot" ]]; then
...
# Authorized users (refer to 'commands.sh: 23') can send internal IRC commands to the IRC server.  (e.g. ReminderBot: injectcmd join #bots auth_key)

elif has "${msg}" "^ReminderBot: injectcmd " && [[ "${AUTHORIZED}" == *"${nick}"* ]] ; then
    cmd=$(echo ${msg} | sed -r 's/^ReminderBot: injectcmd //')
    send "${cmd}"

# Authorized users can send msgs as ReminderBot to other users/channels.  (e.g. ReminderBot: sendcmd #bots test msg)

elif has "${msg}" "^ReminderBot: sendcmd " && [[ "${AUTHORIZED}" == *"${nick}"* ]] ; then
    buffer=$(echo ${msg} | sed -re 's/^ReminderBot: sendcmd //')
